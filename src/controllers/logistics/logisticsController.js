...
const authLogistics = asyncHandler(async (req, res) => {
  ...
  const logistics = await Logistics.findOne({ email: trimmedEmail });

  if (!logistics) {
    res.status(401);
    throw new Error("Logistics company with this email does not exist");
  }
  ...
  const isPasswordValid = await logistics.matchPassword(password);

  if (logistics && isPasswordValid) {
...
const registerLogistics = asyncHandler(async (req, res) => {
  ...
  try {
    const logisticsExists = await Logistics.findOne({ email: trimmedEmail });
  ...
  try {
    ...
    if (logistics) {
      logistics.logisticsName =
        req.body.logisticsName || logistics.logisticsName;
      ...
      if (req.body.schedule) {
        logistics.schedule = JSON.parse(req.body.schedule);
      }

      // Update closed status
      if (req.body.closed) {
        logistics.closed = req.body.closed;
      }
      ...
      res.json({
        ...
        schedule: updatedLogistics.schedule,
        closed: updatedLogistics.closed,
      });
    } else {
      res.status(404);
...
const getLogisticsProfile = asyncHandler(async (req, res) => {
  ...
  if (logistics) {
    res.json({
      ...
      logisticsAddress: logistics.logisticsAddress,
      closed: logistics.closed,
...
const getLogistics = asyncHandler(async (req, res) => {
  const { latitude, longitude, distance, day, time } = req.query;
...
const deleteLogisticsAccount = asyncHandler(async (req, res) => {
  const logistocsId = req.logistics._id;
  ...
  try {
    ...
    logistics.deleted = true;
    logistics.deletedAt = Date.now();
    await logistics.save();
    ...
    res.status(200).json({ message: "Logistics account deleted successfully" });
  } catch (error) {
    console.error("Error deleting logistics account:", error);
    res.status(500).json({ message: "Internal Server Error" });
  }
});

const recoverLogisticsAccount = asyncHandler(async (req, res) => {
  const { email, password } = req.body;

  const logistics = await Logistics.findOne({ email, isDeleted: true });

  if (logistics && (await logistics.matchPassword(password))) {
    const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);

    if (logistics.deletedAt > thirtyDaysAgo) {
      logistics.isDeleted = false;
      logistics.deletedAt = null;
      await logistics.save();
...
const closeLogisticsCompany = asyncHandler(async (req, res) => {
  const logistics = await Logistics.findById(req.params.id);
  if (!logistics) {
    res.status(404).json({ message: "Logistics company not found" });
    return;
  }
  logistics.closed = true;
  await logistics.save();
  res.status(200).json({ message: "Logistics company closed successfully" });
});
