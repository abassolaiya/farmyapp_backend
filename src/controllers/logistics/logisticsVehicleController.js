...
const getAVehicleById = asyncHandler(async (req, res) => {
...
const updateLogisticsVehicle = asyncHandler(async (req, res) => {
  const { vehicleType, plateNum, price, closed } = req.body;
  ...
  if (vehicle) {
    ...
    if (plateNum) {
      ...
      vehicle.slug = slugify(plateNum, { lower: true });
    }
    if (price) vehicle.price = price;
    if (closed !== undefined) vehicle.closed = closed;
...
const searchLogisticsVehicles = asyncHandler(async (req, res) => {
  try {
    const { latitude, longitude, distance, vehicleType, logisticsName } =
      req.body;
            ...
            "officeLocations.distanceKilometers": {
              $let: {
                vars: {
                  lat1: {
                    $arrayElemAt: [
                      "$officeLocations.coordinates.coordinates",
                      1,
                    ],
                  },
                  lon1: {
    ...
    if (logisticsName) {
      const logisticsCompany = logisticsCompanies.find(
        (logistics) => logistics.logisticsName === logisticsName
      );
      if (logisticsCompany) {
        query.userId = logisticsCompany._id;
      }
    }
...
const closeLogisticsVehicle = asyncHandler(async (req, res) => {
  const vehicle = await LogisticsVehicle.findById(req.params.id);
  if (!vehicle) {
  ...
  vehicle.closed = true;
  await vehicle.save();
  res.status(200).json({ message: "Vehicle closed successfully" });
});
