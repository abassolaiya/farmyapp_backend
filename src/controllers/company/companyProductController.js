...
const writeProductReview = asyncHandler(async (req, res) => {
  ...
  const companySlug = req.params.companySlug; // Assuming you're passing companySlug in the route parameter
  const productSlug = req.params.slug; // Assuming you're passing product slug in the route parameter

  // Find the company by its slug to get the userId
  const company = await Company.findOne({ slug: companySlug });
  ...
  // Find the product by company userId and product slug
