...
const approveCompanyCart = async (req, res) => {
  try {
    ...
    let cart = await CompanyCart.findOne({ _id: cartId });
    // let cart = await CompanyCart.findOne({ company: user._id, _id: cartId });
...
const addToCompanyCart = async (req, res) => {
  try {
    ...
    if (!cart) {
      ...
      if (
        cart.company &&
        cart.company.toString() !== productDetails.userId.toString()
      ) {
